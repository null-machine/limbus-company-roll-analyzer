...
class Sinner:
	...
	def gen_display_str(self):
		return f'{self.name} | agg: {round(self.aggregate, 2)} | raw: {round(self.damage, 2)} | var: {round(self.variance, 2)}'
	...
	def gen_chart(self):
		...
		ax[0].set_title(f'{self.s1.name} | agg: {round(self.s1.aggregate, 2)} | raw: {round(self.s1.damage, 2)} | var: {round(self.s1.variance, 2)}')
		...
		ax[1].set_title(f'{self.s2.name} | agg: {round(self.s2.aggregate, 2)} | raw: {round(self.s2.damage, 2)} | var: {round(self.s2.variance, 2)}')
